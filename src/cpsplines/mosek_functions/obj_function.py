...
from src.cpsplines.utils.cholesky_semidefinite import cholesky_semidef
from src.cpsplines.utils.fast_kron import matrix_by_tensor_product, penalization_term
...
class ObjectiveFunction:
    ...
    def _create_var_dict(self, n: int) -> dict[str, mosek.fusion.LinearVariable]:
        ...
        - The coefficient multidimensional array ("theta") with dimensions k_1 x ... x k_N, where k_i is the cardinal of B-spline basis along the i-th dimension.
        - The artificial variable of the design matrix product ("t_B") included in the reformulation of the quadratic term.
        - The artificial variable of the i-th penalty matrix product ("t_D_i") when restating this summand as a rotated quadratic cone constraint.
    ...
    def create_obj_function(
        ...
        if isinstance(family, Gaussian):
            ...
            obj = mosek.fusion.Expr.sub(
                mosek.fusion.Expr.add(mosek.fusion.Expr.sum(self.var_dict["t"]), obj),
            ...
            obj = mosek.fusion.Expr.sub(
                ...
                mosek.fusion.Expr.dot(obj_matrices["y"].astype(float).flatten(), coef),
            )

        # Generate the minimization objective function object
        obj = self.model.objective("obj", mosek.fusion.ObjectiveSense.Minimize, obj)
