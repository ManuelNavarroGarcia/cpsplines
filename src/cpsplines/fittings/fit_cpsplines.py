...
from src.cpsplines.mosek_functions.obj_function import ObjectiveFunction
from src.cpsplines.mosek_functions.pdf_constraints import PDFConstraint
from src.cpsplines.mosek_functions.point_constraints import PointConstraints
...
from src.cpsplines.utils.fast_kron import matrix_by_transpose
from src.cpsplines.utils.gcv import GCV
from src.cpsplines.utils.normalize_data import DataNormalizer
from src.cpsplines.utils.rearrange_data import RearrangingError, scatter_to_grid
from src.cpsplines.utils.simulator_grid_search import print_grid_search_results
from src.cpsplines.utils.simulator_optimize import Simulator
...
class CPsplines:
    ...
    def _initialize_model(
        ...
        data_normalizer : Optional[DataNormalizer]
            The DataNormalizer object if `y_range` is not None and None otherwise. By default, None.
    ...
    def fit(self, data: pd.DataFrame, y_col: str, y_range: Iterable[int | float] | None = None, **kwargs):
        ...
        y_range : Optional[Iterable[Union[int, float]]]
            If not None, `y` is scaled in the range defined by this parameter. This scaling process is useful when `y` has very large norm, since MOSEK may not be able to find a solution in this case due to numerical issues. By default, None.
        ...
        if data.shape[1] > 2:
            ...
            # When out-of-sample prediction is considered, the convex hull must be extended till the prediction horizon for the whole range of the remaining variables
            if self.x_range:
        ...
        # Filling the arguments of the method used to determine the optimal set of smoothing parameters
