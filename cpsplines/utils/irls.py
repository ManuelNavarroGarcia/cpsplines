...
def irls(
    ...
    if X.shape[0] != y.shape[0]:
        raise ValueError("The design matrix `X` and the response vector `y` have different number of rows.")
    ...
    for iter in range(max_iter):
        ...
        beta = np.linalg.solve(X.T @ utils_irls["W"] @ X, X.T @ utils_irls["W"] @ utils_irls["Z"])
