...
from src.cpsplines.utils.fast_kron import matrix_by_tensor_product
...
def test_matrices_by_tensor(common_dim, different_dim):
    ...
    with timer(tag="Using np.kron"):
        exp_out = (random_tensor.flatten() @ reduce(np.kron, random_mat)).reshape(*different_dim)

    with timer(tag="Using folding and unfolding"):
        out = matrix_by_tensor_product(matrices=[A.T for A in random_mat], T=random_tensor)
